...
static int compara_vocabula(const void *a, const void *b)
{
    ...
    return strcmp(g_lex_ordinatio->vocabula[ia].chorda,
...
int nm_lexator_exercita(nm_lexator_t *lex, const char *corpus)
{
    ...
    while (lex->vocab_magnitudo < lex->vocab_max) {
        ...
        /* numeratio parium */
        for (int i = 0; i < n - 1; i++) {
            if (signa[i] < 0 || signa[i+1] < 0) continue;
        ...
        if (best_freq <= 1) break; /* nihil fundit */
        ...
        if (la + lb >= NM_LEX_VOCABULUM_MAX) break;
        memcpy(lex->vocabula[nou].chorda,
               lex->vocabula[best_a].chorda, la);
        memcpy(lex->vocabula[nou].chorda + la,
...
int nm_lexator_serva(const nm_lexator_t *lex, const char *via)
{
    ...
    for (int i = 0; i < lex->vocab_magnitudo; i++) {
        ...
        fwrite(lex->vocabula[i].chorda,     1,
...
int nm_lexator_lege(nm_lexator_t *lex, const char *via)
{
    ...
    if (fread(&magicus, sizeof(unsigned int), 1, f) != 1 ||
        magicus != NM_LEX_SIGNUM_MAGICUM) {
        fclose(f); return -1;
    }

    int vm, mvl;
    if (fread(&vm,  sizeof(int), 1, f) != 1 ||
    ...
    for (int i = 0; i < vm; i++) {
        float gradus; int lon;
        if (fread(&gradus, sizeof(float), 1, f) != 1 ||
                        ...
                        int bos, int eos, int *signa, int *n_signa)
{
    ...
    if (bos) signa[n++] = 1; /* signum initii (conventio: index 1) */
    ...
    while (mutatum) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            if (la + lb >= NM_LEX_VOCABULUM_MAX) continue;
    ...
    if (eos) signa[n++] = 2; /* signum finis (conventio: index 2) */
    *n_signa = n;
}
...
{
    if (signum < 0 || signum >= lex->vocab_magnitudo) return "?";
