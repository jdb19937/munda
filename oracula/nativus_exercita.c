...
int main(int argc, char **argv)
{
    ...
    if (!lex) {
        ...
        free(corpus); pfr_computo_fini(); return 1;
    }
    ...
    int resume = 0;
    {
        ...
        if (fp && nm_lexator_lege(lex, via_lex) == 0) {
            printf("2. lexator oneratus ex '%s' (vocab=%d)\n\n",
            ...
            if (nm_lexator_exercita(lex, corpus) < 0) {
                ...
                free(corpus); nm_lexator_destrue(lex);
                pfr_computo_fini(); return 1;
            }
    ...
    if (resume && nm_lege(&nm, via_bin) == 0) {
        printf("3. exemplar oneratum ex '%s' (dim=%d strata=%d vocab=%d)\n\n",
        ...
        printf("3. initio exemplar novum (dim=%d occ=%d strata=%d capita=%d"
               " kv=%d vocab=%d lm=%d)...\n",
        ...
        if (nm_initia_temere(&nm, &config, 42u) < 0) {
            ...
            free(signa); nm_lexator_destrue(lex);
            pfr_computo_fini(); return 1;
        }
    ...
    printf("4. exercito (%d passus, seq_lon=%d, mini=%d, tutela=%d)...\n",
    ...
    if (nm_exercitatio_initia(&ex, &nm.config) < 0) {
        ...
        nm_fini(&nm); free(signa); nm_lexator_destrue(lex);
        pfr_computo_fini(); return 1;
    }
    ...
    while (passus_factus < n_passus) {
        ...
        exercita(&nm, &ex, signa, n_signa, batch, DEF_SEQ_LON,
                 passus_factus, &semen);
        passus_factus += batch;

        /* tutela intermedia */
        if (nm_serva(&nm, via_bin) == 0)
            printf("  [tutela: passus %d/%d -> %s]\n",
